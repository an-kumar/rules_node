...
def _get_dirpath(module, file):
  parts = file.path.partition("/%s/" % module.name)
  return parts[0] + parts[1]

def _nodemodule_path(dstpath):
  parts = dstpath.partition("/node_modules/")
  return parts[0] + parts[1]

def _numcomponents(module, srcpath):
  parts = srcpath.path.partition("/%s/" % module.name)
  return len(parts[0].split("/"))



def _symlink_module(ctx, target_dir, module):
    ...
    script_file = ctx.new_file('%s/tarcopy_%s.sh' % (ctx.label.name, module.identifier))
    ...
    dirpath = _get_dirpath(module, srcfiles[0])
    numcomponents = _numcomponents(module, srcfiles[0])

    for src in module.files:
      src_dirpath = _get_dirpath(module, src)
      if src_dirpath != dirpath:
        fail("Dirpaths didnt match; %s vs %s" % (dirpath, src_dirpath))
      src_numcomponents = _numcomponents(module, src)
      if src_numcomponents != numcomponents:
        fail("num components didnt match; %d vs %d" % (numcomponents, src_numcomponents))

    
    tarball = '%s/tarfile_%s.tar' % (ctx.label.name, module.identifier)
    ...
    script_line = 'tar -cf %s %s -C %s' % (tarball_file.path, ' '.join(source_files), dirpath)
    script_line2 = 'tar -xf %s --strip-components=%d -C %s' % (tarball_file.path, numcomponents, _nodemodule_path(dstpath))
    script_lines.append(script_line)
    script_lines.append(script_line2)
    ...
    ctx.action(
        mnemonic = 'TarCopyModuleWith%sFiles' % len(outputs),
        inputs = inputs + [script_file],
        outputs = outputs + [tarball_file],
...
def _symlink_modules(ctx, target_dir, deps):
    outputs = []
    for dep in deps:
        module = dep.node_module
        outputs += _symlink_module(ctx, target_dir, module)
        for module in module.transitive_deps:
            outputs += _symlink_module(ctx, target_dir, module)
    return outputs
...
def node_modules_impl(ctx):
    ...
    if ctx.attr.symlink:
      files = _symlink_modules(ctx, target_dir, ctx.attr.deps)
    else:
